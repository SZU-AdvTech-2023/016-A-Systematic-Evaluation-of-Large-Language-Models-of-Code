...
class Enron(DataDownloader):
    ...
    def exists(self):
        """Check if the local dataset is present"""
        # Replace 'local_dataset_path' with the path to your local dataset
        local_dataset_path = '/data1/dxl/code/gpt-neox/data/enron/enron_emails.jsonl.zst'
        return os.path.isfile(local_dataset_path)

    def prepare(self):
        """Prepare the dataset using local files"""
        if not self.exists():
            print("Dataset not found. Please place the dataset in the expected directory.")
            return
        # The rest of the preparation process (tokenization, etc.)
        self.tokenize()
...
class Enwik8(DataDownloader):
    ...
    def exists(self):
        """Check if the local dataset is present"""
        # Replace 'local_dataset_path' with the path to your local dataset
        local_dataset_path = '/data1/dxl/code/gpt-neox/data/enwik8/enwik8'
        return os.path.isfile(local_dataset_path)
